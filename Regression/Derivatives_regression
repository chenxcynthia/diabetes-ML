...
# The dataset for the derivatives method needs to be altered, because the patients' different timepoints depend on each other. 

# We define 2 classes for now:
...
#sort features by  patient ID so every patient's data is grouped together, this makes it easier to take derivatives
#df_metabolomics.transpose(inplace = True)
#df_metabolomics.sort_index(inplace = True)
#df_mapping.sort_values('ID', inplace = True)
...
new_dim = 20#len(X) - 1 #new_dim is number of reduced features (after dimensionality reduction)
...
# ### Compute RMSE error and coefficients:
...
print('RMSE on training: %.4f' %rmse_train)
print('RMSE on 10-fold cross validation: %.4f' %rmse_10cv)
# Explained variance score: 1 is perfect prediction
